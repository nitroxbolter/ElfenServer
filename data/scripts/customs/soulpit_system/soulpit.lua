...
local event = {
    waves = {
        ...
        [28337] = { 
            monsters = {
                ...
                {"furious elder bloodjaw", "furious elder bloodjaw", "furious elder bloodjaw"},
                {"savage elder bloodjaw", "savage rotten man-maggot"},
                {"wrathful elder bloodjaw"}
            },
                storage = 876558,
                waves = 5,
                name = "Elder Bloodjaw"
        ...
        [28354] = {
            monsters = {
                {"angry hazardous phantom", "angry hazardous phantom", "angry hazardous phantom", "angry hazardous phantom"},
                {"enraged hazardous phantom", "enraged hazardous phantom", "enraged hazardous phantom"},
                {"furious hazardous phantom", "furious hazardous phantom", "furious hazardous phantom"},
                {"savage hazardous phantom", "savage hazardous phantom"},
                {"wrathful hazardous phantom"},
            },
                storage = 876572,
                waves = 5,
    ...
    spawnArea = {Position(31527, 32257, 8), Position(31550, 32262, 8)},
    creatures = {},
    currentWave = 0,
    participants = {},
    currentItem = nil
    ...
    if event.currentItem then
        if event.waves[event.currentItem] and event.currentWave == event.waves[event.currentItem].waves then
            ...
            for _, playerId in pairs(event.participants) do
                ...
                if player then
                    player:setStorageValue(event.waves[event.currentItem].storage, 1)
...
local config = {
    actionId = 57392,
    participantsPos = {
        Position(31588, 32270, 8),
        Position(31588, 32271, 8),
        Position(31588, 32272, 8),
        Position(31588, 32273, 8),
        Position(31588, 32274, 8)
    },
    spawnArea = {
    fromPos = Position(31526, 32253, 8), 
    toPos = Position(31548, 32268, 8)
    },
    entranceArea = Position(31538, 32267, 8),
}
...
function soulPit.onUse(player, item, fromPos, target, toPos, isHotkey)
    ...
    for _, spectator in pairs(spectators) do
        if spectator:isPlayer() then
            player:sendTextMessage(MESSAGE_INFO_DESCR, "At this time the Soulpit is occupied, please try again later.")
            return true
        end
    end
    ...
    if event.waves[item:getId()] and target:getActionId() == config.actionId then
        ...
        if checkAndStartWave(player, item) then
            for i = 5, 1, -1 do
                addEvent(function(currentCount)
                    for _, spectator in ipairs(spectators) do
                        spectator:say(tostring(currentCount), TALKTYPE_MONSTER_SAY, false, spectator, Position(31588, 32269, 8))
                    end
                end, (5 - i) * 1000, i)
            end
            ...
            addEvent(function()
                ...
                target:transform(28240)
            end, 5000)
        end
    else
        player:sendTextMessage(MESSAGE_INFO_DESCR, "You need a Soul Core to start the event.")
    end
    return true
end
