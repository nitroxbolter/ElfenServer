local npc1Position = Position(30827, 31225, 7)
...
local function spawnNpc(npcId, position)
    npcs[npcId] = Game.createNpc("Sayge", position)
    if npcs[npcId] then
...
for i, time in ipairs(spawnTimes) do
    local coassitExpedition = GlobalEvent("SaygeExpedition" .. i)
            ...
            for y = fromPosX.y, toPosX.y do
                ...
                if tile then
                    ...
                    if creature and creature:isPlayer() then
                        ...
                        creature:addMapMark(Position(30827, 31225, 7), 22, "Expedition: Spider Nest's")
                        Game.broadcastMessage("Sayge has spawned on Frosthold island.", MESSAGE_EVENT_ADVANCE)
                    end
                end
            end
        end
