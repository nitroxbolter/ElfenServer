local monsterPosition = Position(30938, 31267, 9)
local monsterType = "Emberlord Zanarth"

local eventDuration = 60 * 60 * 1000
local fromPos = Position(30923, 31254, 9)
local toPos = Position(30953, 31282, 9)
        ...
        for y = fromPos.y, toPos.y do
            ...
            if tile then
                ...
                if creature and creature:isPlayer() then
                    ...
                    creature:addMapMark(monsterPosition, 23, "Boss: Emberlord Zanarth")
                    Game.broadcastMessage("The Boss " .. monsterType .. " has spawned Deep on Vyrsk Island.", MESSAGE_EVENT_ADVANCE)
                end
            end
        end
    end
        ...
        for y = areaFrom.y, areaTo.y do
            ...
            if tile then
                ...
                if creature and creature:isMonster() and creature:getName():lower() == monsterName:lower() then
                    creature:remove()
                    Game.broadcastMessage("The Boss " .. monsterName .. " has vanished from Deep Vyrsk Island.", MESSAGE_EVENT_ADVANCE)
                end
            end
        end
    end
...
for i, time in ipairs(spawnTimes) do
    local monsterEvent = GlobalEvent("SummonEmberlord" .. i)
