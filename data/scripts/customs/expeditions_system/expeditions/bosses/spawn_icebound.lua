local monsterPosition = Position(30911, 31307, 4)
local monsterType = "Icebound Thryssan"

local eventDuration = 60 * 60 * 1000
local fromPos = Position(30896, 31293, 4)
local toPos = Position(30923, 31321, 4)
        ...
        for y = fromPos.y, toPos.y do
            ...
            if tile then
                ...
                if creature and creature:isPlayer() then
                    ...
                    creature:addMapMark(monsterPosition, 23, "Boss: Icebound Thryssan")
                    Game.broadcastMessage("The Boss " .. monsterType .. " has spawned on Frosthold Island.", MESSAGE_EVENT_ADVANCE)
                end
            end
        end
    end
        ...
        for y = areaFrom.y, areaTo.y do
            ...
            if tile then
                ...
                if creature and creature:isMonster() and creature:getName():lower() == monsterName:lower() then
                    creature:remove()
                    Game.broadcastMessage("The Boss " .. monsterName .. " has vanished from Frosthold Island.", MESSAGE_EVENT_ADVANCE)
                end
            end
        end
    end
...
for i, time in ipairs(spawnTimes) do
    local monsterEvent = GlobalEvent("SummonIceBound" .. i)
