local monsterPosition = Position(31635, 31165, 6)
local monsterType = "Tidebreaker Voryn"

local eventDuration = 60 * 60 * 1000
local fromPos = Position(31531, 31143, 6)
local toPos = Position(31681, 31197, 6)
        ...
        for y = fromPos.y, toPos.y do
            ...
            if tile then
                ...
                if creature and creature:isPlayer() then
                    ...
                    creature:addMapMark(monsterPosition, 23, "Boss: Tidebreaker Voryn")
                    Game.broadcastMessage("The Boss " .. monsterType .. " has spawned on Vardenfell Island.", MESSAGE_EVENT_ADVANCE)
                end
            end
        end
    end
        ...
        for y = areaFrom.y, areaTo.y do
            ...
            if tile then
                ...
                if creature and creature:isMonster() and creature:getName():lower() == monsterName:lower() then
                    creature:remove()
                    Game.broadcastMessage("The Boss " .. monsterName .. " has vanished from Vardenfell Island.", MESSAGE_EVENT_ADVANCE)
                end
            end
        end
    end
...
for i, time in ipairs(spawnTimes) do
    local monsterEvent = GlobalEvent("SummonTidebreaker" .. i)
