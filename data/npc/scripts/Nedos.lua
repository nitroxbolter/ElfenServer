...
local shop = {
    ...
    {id=29511, buy=40, sell=20, name='radiant topaz shard'},
    {id=29532, buy=60, sell=30, name='thunder prism'},
    {id=29533, buy=60, sell=30, name='static core'},
    {id=29505, buy=85, sell=40, name='solar draught'},
    {id=29517, buy=100, sell=50, name='blazing topaz shard'},
    {id=29523, buy=170, sell=85, name='stormheart globe'},
...
local function onBuy(cid, item, subType, amount, ignoreCap, inBackpacks)
    ...
    local playerStorage = player:getStorageValue(48076)
    
    if playerStorage < totalCost then
        return player:sendTextMessage(MESSAGE_INFO_DESCR, "You don't have enough vardenfell coins.")
    end
    ...
    player:setStorageValue(48076, playerStorage - totalCost)
    player:addItem(itemsTable[item].id, amount)
    player:sendTextMessage(MESSAGE_INFO_DESCR,
        "Bought "..amount.."x "..itemsTable[item].name.." for "..totalCost.." vardenfell coins.")
...
local function onSell(cid, item, subType, amount, ignoreCap, inBackpacks)
    ...
    player:setStorageValue(48076, math.max(0, player:getStorageValue(48076)) + totalValue)
    player:removeItem(itemsTable[item].id, amount)
    player:sendTextMessage(MESSAGE_INFO_DESCR,
        "Sold "..amount.."x "..itemsTable[item].name.." for "..totalValue.." vardenfell coins.")
    sendReputationToPlayer(player)
    return true
end
...
local voices = {
    { text = 'Vardenfell Reputation Coins' },
}
...
function creatureSayCallback(cid, type, msg)
    ...
    if msgcontains(msg, 'trade') then
        ...
        local coinAmount = player:getStorageValue(48076) or 0
...
npcHandler:setMessage(MESSAGE_GREET, 'Hello |PLAYERNAME|, we {trade} vardenfell coins for some valuables.')
