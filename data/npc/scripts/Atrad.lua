...
function creatureSayCallback(cid, type, msg)
	...
	elseif(msgcontains(msg, "nose ring") or msgcontains(msg, "ring")) then
		if(getPlayerStorageValue(cid, 18999) == 1) then
			...
			PlayerRemoveItem(cid, 5804, 1)
			PlayerRemoveItem(cid, 5930, 1)
			PlayerAddOutfit(cid, getPlayerSex(cid) == 0 and 156 or 152, 2)
			PlayerSetStorageValue(cid, 18999, 2) -- exaust
	...
	elseif(msgcontains(msg, "yes")) then
		...
		elseif(npcHandler.topic[cid] == 3) then
				...
				PlayerSetStorageValue(cid, 18999, 1)
				npcHandler.topic[cid] = 0
		end
	end
	return true
