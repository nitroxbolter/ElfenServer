...
local shop = {
    ...
    {id=29512, buy=40, sell=20, name='twilight amethyst shard'},
    {id=29534, buy=60, sell=30, name='melodic shard'},
    {id=29535, buy=60, sell=30, name='phantom resonance'},
    {id=29506, buy=85, sell=40, name='amethyst vial'},
    {id=29518, buy=100, sell=50, name='eternal amethyst shard'},
    {id=29524, buy=170, sell=85, name='ethereal harmony orb'},
...
local function onBuy(cid, item, subType, amount, ignoreCap, inBackpacks)
    ...
    local playerStorage = player:getStorageValue(48079)
    
    if playerStorage < totalCost then
        return player:sendTextMessage(MESSAGE_INFO_DESCR, "You don't have enough hallowfall coins.")
    end
    ...
    player:setStorageValue(48079, playerStorage - totalCost)
    player:addItem(itemsTable[item].id, amount)
    player:sendTextMessage(MESSAGE_INFO_DESCR,
        "Bought "..amount.."x "..itemsTable[item].name.." for "..totalCost.." hallowfall coins.")
...
local function onSell(cid, item, subType, amount, ignoreCap, inBackpacks)
    ...
    player:setStorageValue(48079, math.max(0, player:getStorageValue(48079)) + totalValue)
    player:removeItem(itemsTable[item].id, amount)
    player:sendTextMessage(MESSAGE_INFO_DESCR,
        "Sold "..amount.."x "..itemsTable[item].name.." for "..totalValue.." hallowfall coins.")
    sendReputationToPlayer(player)
    return true
end
...
local voices = {
    { text = 'Hallowfall Reputation Coins' },
}
...
function creatureSayCallback(cid, type, msg)
    ...
    if msgcontains(msg, 'trade') then
        local player = Player(cid)
        player:sendExtendedOpcode(ServerOpcodes.currencyType, "H")
        local coinAmount = player:getStorageValue(48079) or 0
...
npcHandler:setMessage(MESSAGE_GREET, 'Hello |PLAYERNAME|, we {trade} hallowfall coins for some valuables.')
