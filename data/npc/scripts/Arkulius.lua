	...
	elseif msgcontains(msg, "mission") or msgcontains(msg, "quest") then
		...
		if value < 1 then
			if player:getLevel() >= 80 then
				...
				elseif player:isIllusionist() then
					npcHandler:say({
						"Okay, listen closely: Illusionist has not been added to do this quest, soon will be available."
					}, cid)
				end
			else
		...
		elseif value == 1 then
			if player:getItemCount(player:isSorcerer() or player:isIllusionist() and 8304 or player:isDruid() and 8305 or player:isPaladin() and 8300 or player:isKnight() and 8306) > 0 then
				...
				npcHandler:say("You need some kind of pure elemental soil from the " .. (player:isSorcerer() or player:isIllusionist() and "Fire" or player:isDruid() and "Earth" or player:isPaladin() and "Ice" or player:isKnight() and "Energy") .. " Overlord. Come back when you've got it.", cid)
			end
			npcHandler.topic[cid] = 0
		elseif value == 2 then
			if player:removeItem(8310, 1) and player:getStorageValue(Storage.ElementalSphere.QuestLine) < 3 then
				...
				player:addItem(player:isSorcerer() or player:isIllusionist() and 8867 or player:isDruid() and 8869 or player:isPaladin() and 8853 or player:isKnight() and 8883, 1)
