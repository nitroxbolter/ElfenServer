...
function creatureSayCallback(cid, type, msg)
	...
	if(msgcontains(msg, "abandoned sewers")) then
		...
		elseif(getPlayerStorageValue(cid, 20062) == 21) then
            ...
            PlayerSetStorageValue(cid, 10050, 4)
			PlayerSetStorageValue(cid, 20053, 1)
            PlayerSetStorageValue(cid, 20054, 0)
	...
	elseif(msgcontains(msg, "yes")) then
		if(npcHandler.topic[cid] == 2) then
			...
			PlayerSetStorageValue(cid, 10062, 1)
			PlayerSetStorageValue(cid, 20062, 0)
	...
	elseif(msgcontains(msg, "ok")) then
		if(npcHandler.topic[cid] == 3) then
			...
			npcHandler.topic[cid] = 0
			PlayerSetStorageValue(cid, 20062, 21)
	...
	if(getPlayerStorageValue(cid, 10050) < 6) then
		--if(npcHandler.topic[cid] == 7) then
			...
			PlayerSetStorageValue(cid, 20055, 1)
			PlayerSetStorageValue(cid, 20056, 0)
			npcHandler.topic[cid] = 0
