...
function creatureSayCallback(cid, type, msg)
	...
	------------------------ FINISH MISSION 02 ------------------------
		...
		elseif(getPlayerStorageValue(cid, 41600) == 5) and getPlayerStorageValue(cid, 41660) == 4 then
			npcHandler:say("You did it!! And I assume you took only what you needed? Heh. No, I know it. Because my men took the rest. Thanks for helping us, you did a very good job. In fact I have a little 'extra' for you here, thanks again.", cid)
				------------------ RESET STORAGE --------------------
				PlayerSetStorageValue(cid, 41600, - 1) -- reset storage
				PlayerSetStorageValue(cid, 41500, - 1) -- reset storage
				PlayerSetStorageValue(cid, 41650, - 1) -- reset storage
				PlayerSetStorageValue(cid, 41660, - 1) -- reset storage
				-----------------------------------------------------
				PlayerSetStorageValue(cid, 43600, os.time() + 20 * 60 * 60) -- set time to start mission again
				PlayerSetStorageValue(cid, 42620, 2) -- quest log
			------------------- ITEM RANDOM --------------------
			items = { 
	...
	elseif(msgcontains(msg, "yes")) then
		...
		elseif(npcHandler.topic[cid] == 2) then
			...
			PlayerSetStorageValue(cid, 41600, 1)
			PlayerSetStorageValue(cid, 42600, 1) -- quest log
		...
		elseif(npcHandler.topic[cid] == 3) and getPlayerItemCount(cid, 13160) >= 7 then
			...
			PlayerRemoveItem(cid, 13160, 7)
			PlayerSetStorageValue(cid, 41600, 2) 
			PlayerSetStorageValue(cid, 42600, 2) -- quest log
			npcHandler.topic[cid] = 0
		...
		elseif(npcHandler.topic[cid] == 4) then 
			...
			PlayerSetStorageValue(cid, 41600, 3) 
			PlayerSetStorageValue(cid, 42610, 1) -- quest log
			npcHandler.topic[cid] = 0
		elseif(npcHandler.topic[cid] == 5) then 
			...
			PlayerSetStorageValue(cid, 41600, 4) 
			PlayerSetStorageValue(cid, 42610, 2) -- quest log
		...
		elseif(npcHandler.topic[cid] == 7) then 
			...
			PlayerSetStorageValue(cid, 41600, 5) 
			PlayerSetStorageValue(cid, 42620, 1) -- quest log
			npcHandler.topic[cid] = 0
		------------------------ FINISH MISSION 03 ------------------------
		end
	end
