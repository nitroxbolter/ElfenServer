...
function Player.canCarryMoney(self, amount)
	...
	-- Add demoniac coins to totalWeight and inventorySlots
	local type_demoniac = ItemType(ITEM_DEMONIAC_COIN)
	local demoniacCoins = math.floor(amount / 100000000)
	if demoniacCoins > 0 then
		amount = amount - (demoniacCoins * 100000000)
		while demoniacCoins > 0 do
			local count = math.min(100, demoniacCoins)
			totalWeight = totalWeight + type_demoniac:getWeight(count)
			demoniacCoins = demoniacCoins - count
...
function Player.isIllusionist(self)
	return table.contains({VOCATION.ID.ILLUSIONIST, VOCATION.ID.ARCH_ILLUSIONIST}, self:getVocation():getId())
end
