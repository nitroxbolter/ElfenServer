...
int32_t Player::getArmor() const
{
	...
	static const slots_t armorSlots[] = {CONST_SLOT_HEAD, CONST_SLOT_NECKLACE, CONST_SLOT_ARMOR, CONST_SLOT_LEGS, CONST_SLOT_FEET, CONST_SLOT_RING, CONST_SLOT_DECK, CONST_SLOT_BADGE, CONST_SLOT_SHIP, CONST_SLOT_BELT, CONST_SLOT_GLOVES };
...
uint32_t Player::getAttackSpeed() const
{
	...
	uint32_t ret = weapon ? weapon->getAttackSpeed() : 2000;
...
ReturnValue Player::queryAdd(int32_t index, const Thing& thing, uint32_t count, uint32_t flags, Creature*) const
{
	...
	if ((slotPosition & SLOTP_HEAD) || (slotPosition & SLOTP_NECKLACE) ||
	        ...
	        (slotPosition & SLOTP_RING) || (slotPosition & SLOTP_DECKBAD) || (slotPosition & SLOTP_SHIP) || (slotPosition & SLOTP_BELTGLOVES)) {
	...
	switch (index) {
		...
		case CONST_SLOT_DECK: {
			if (slotPosition & SLOTP_DECKBAD) {
				ret = RETURNVALUE_NOERROR;
			}
			break;
		}

		case CONST_SLOT_BADGE: {
			if (slotPosition & SLOTP_DECKBAD) {
				ret = RETURNVALUE_NOERROR;
			}
			break;
		...
		case CONST_SLOT_SHIP: {
			if (slotPosition & SLOTP_SHIP) {
				ret = RETURNVALUE_NOERROR;
			}
			break;
		}
		...
		case CONST_SLOT_BELT: {
			if (slotPosition & SLOTP_BELTGLOVES) {
				ret = RETURNVALUE_NOERROR;
			}
			break;
		}
...
bool Player::updateSaleShopList(const Item* item)
{
	...
	if (itemId != ITEM_GOLD_COIN && itemId != ITEM_PLATINUM_COIN && itemId != ITEM_CRYSTAL_COIN && itemId != ITEM_AMBER_COIN && itemId != ITEM_DEMONIAC_COIN) {
...
void Player::onAddCombatCondition(ConditionType_t type)
{
	switch (type) {
		...
		case CONDITION_SPLASHED:
			sendTextMessage(MESSAGE_STATUS_DEFAULT, "You are splashed.");
			break;
