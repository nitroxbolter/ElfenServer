...
static constexpr int32_t PLAYER_MAX_SPEED = 12000;
...
class Player final : public Creature, public Cylinder
{
	public:
		...
		bool removeWings(uint8_t wingId);
		...
		int32_t getMaxHealth() const override {
			...
			return std::max<int64_t>(1, healthMax + varStats[STAT_MAXHITPOINTS] + vitHealth);
		}
		...
		uint32_t getMaxMana() const {
			...
			return std::max<int64_t>(0, manaMax + varStats[STAT_MAXMANAPOINTS] + sprMana);		}
